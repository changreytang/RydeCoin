...
class HttpServer {
    constructor(node, blockchain, operator, miner) {
        ...
        this.app.post('/rydecoin/create_account', (req, res) => {
            ...
            if (R.match(/\w+/g, password).length <= 4) {
                ...
                try {
                    ...
                    let newTransaction = operator.createTransaction(managerWalletId, managerAddress, newAddress, 100, managerAddress);
        ...
        this.app.post('/rydecoin/book_ride/',(req,res) => {
            console.log("Book Ride Request Recieved");
            let walletId = req.body.walletId;
            let password = req.body.password;

            if(password == null) throw new HTTPError(401, 'Wallet\'s password is missing !');
            
            let fromAddress = req.body.addressId;
            let rideId = req.body.rideId;
            let amount = req.body.amount;
            //console.info(`Booking the ride :`+walletId+'....'+addressId+'....'+rideId+'..'+amount);
            let walletAddresses = operator.getAddressesForWallet(walletId);
            let passwordHash = CryptoUtil.hash(password);
            ...
            try{
                let toAddress = operator.getManagerAddress();

                if (!operator.checkWalletPassword(walletId, passwordHash)) throw new HTTPError(403, `Invalid password for wallet '${walletId}'`);

                let newTransaction =operator.createTransaction_v1(walletId, fromAddress , toAddress, amount ,req.body.fromAddress , rideInfo , 'book_ride');

                newTransaction.check();
                
                let transactionCreated = blockchain.addTransaction(Transaction.fromJson(newTransaction));
                
                res.redirect(200, '/rydecoin');
        ...
        this.app.post('/rydecoin/finish_ride/', (req, res) => {
            console.log("Finish Ride Request Recieved");
            let walletId = req.body.walletId;
            let password = req.body.password;
            let walletAddresses = operator.getAddressesForWallet(walletId);
            if (walletAddresses.indexOf(req.body.addressId) === -1)
                throw new HTTPError(403, `Invalid address for wallet '${walletId}'`);
            ...
            try {
                ...
                //Find the relevant trigger transaction and get values

                let amount = blockchain.getRideTransaction(req.body.rideId,req.body.addressId);
                let fromAddress = operator.getManagerAddress();
                let operatorWalletId = operator.getManagerWalletId();
                let rideInfo =  {
                    "rideId":req.body.rideId
                };
                let newTransaction = operator.createTransaction_v1(operatorWalletId, fromAddress, req.body.addressId, 
                    amount, fromAddress, rideInfo, 'finish_ride');
        ...
        this.app.post('/operator/wallets/:walletId/finishRide', (req, res) => {
            console.log('finishRide Api');
            let walletId = req.params.walletId;
            let password = req.body.password;
            let walletAddresses = operator.getAddressesForWallet(walletId);
            if (walletAddresses.indexOf(req.body.userAddress) === -1)
                throw new HTTPError(403, `Invalid address for wallet '${walletId}'`);
            ...
            try {
                ...
                let amount = blockchain.getRideTransaction(req.body.rideId,req.body.userAddress);
                let fromAddress = operator.getManagerAddress();
                let operatorWalletId = operator.getManagerWalletId();
                let newTransaction = operator.createTransaction_v1(operatorWalletId, fromAddress, req.body.userAddress, 
                    amount, fromAddress, {'rideId':req.body.rideId}, 'finish_ride');
