...
class Blockchain {
    ...
    getAppState() {
        ...
        blocks.forEach(function(block) {
            block.transactions.forEach(function(tr){
                if (tr.type === 'create_ride'){
                    //Update state
                    var state_obj = tr.meta_data;
                    state_obj['occupied'] = 0;
                    state_obj['finished'] = 0;
                    state_obj['ride_manager'] = tr.data.outputs[0].address;
                    state.push(state_obj);
                }
                else if(tr.type === 'book_ride'){
                    //The ride type can change.
                    console.log( 'here...'+tr.meta_data.rideId);
                    var idx = R.findIndex(R.propEq('rideId', tr.meta_data.rideId),state);
                    console.log('here...2')
                    if(idx > -1)
                        state[idx]['occupied'] += 1;
                }
                else if(tr.type === 'finish_ride'){
                    //The ride type can change.
                    var idx = R.findIndex(R.propEq('rideId', tr.meta_data.rideId),state);
                    if(idx > -1){
                        state[idx]['finished'] += 1;
                        if(state[idx]['finished'] === parseInt(state[idx]['space'])+1){
                            //This means all the riders and driver have completed the ride
                            //Remove it from state now.
                            state.splice(idx, 1);
                        }
                    }                        
    ...
    logNumberOfCommittedTxns() {
        let numTxnsCommitted = 0;
        let blocks = this.blocks;
        blocks.forEach(function(block) {
            block.transactions.forEach(function(txn) {
                if (txn.type == 'regular') numTxnsCommitted += 1;
            });
        });
        fs.appendFileSync(`experiments/${this.dbName}.log`,
                          `${new Date().toUTCString()}: ${numTxnsCommitted}\n`);
    }

    getRideTransaction(rideId,fromAddress) {
        let blocks = this.blocks;
        let return_amt = 0;
        blocks.forEach(function(block) {
            block.transactions.forEach(function(tr){
                if (tr.type === 'create_ride' || tr.type === 'book_ride'){
                    //Update state
                    //Check if rideId and fromaddress present in input transactions match
                    if (tr.meta_data.rideId === rideId && tr.data.inputs[0].address === fromAddress){
                        return_amt = tr.data.outputs[0].amount;
                    }
                }
            });
        });
    ...
    replaceChain(newBlockchain) {
        ...
        // console.info(`NEW BLOCKCHAIN: ${JSON.stringify(newBlockchain)}`)

        // Verify if the new blockchain is correct
        this.checkChain(newBlockchain);

        // try {
        //     fs.unlinkSync('data/' + this.dbName + '/' + BLOCKCHAIN_FILE);
        //     fs.unlinkSync('data/' + this.dbName + '/' + TRANSACTIONS_FILE);
        //     fs.writeFileSync('data/' + this.dbName + '/' + BLOCKCHAIN_FILE, '[]')
        //     fs.writeFileSync('data/' + this.dbName + '/' + TRANSACTIONS_FILE, '[]')
        ...
        //     this.transactions = this.transactionsDb.read(Transactions);
        //     console.info('Successfully restarted blockchain and transactions')
        // } catch (err) {
        //     console.error(`Could not delete blocks.json: ${err}`)
        // }
        ...
        // let newBlocks = R.takeLast(newBlockchain.length - this.blocks.length + 2, newBlockchain);
        // let newBlocks = R.takeLast(newBlockchain.length, newBlockchain);
    ...
    addBlock(newBlock, emit = true) {
        ...
        while (this.checkBlock(newBlock, this.getLastBlock()) == false) {
            console.info('REMOVING BLOCKING!!!');
            this.blocks.pop();
        }
    ...
    checkBlock(newBlock, previousBlock, referenceBlockchain = this.blocks) {
        ...
        if (previousBlock.index + 1 !== newBlock.index) { // Check if the block is the last one
            ...
            throw new BlockAssertionError(`Invalid index: expected '${previousBlock.index + 1}' got '${newBlock.index}'`); // NOT NEEDED?
            ...
            throw new BlockAssertionError(`Invalid previoushash: expected '${previousBlock.hash}' got '${newBlock.previousHash}'`); // NOT NEEDED?
            ...
            throw new BlockAssertionError(`Invalid hash: expected '${blockHash}' got '${newBlock.hash}'`); // NOT NEEDED?
            ...
            throw new BlockAssertionError(`Invalid proof-of-work difficulty: expected '${newBlock.getDifficulty()}' be smaller than '${this.getDifficulty()}'`); // NOT NEEDED?
        }
        ...
        if (newBlock.transactions.length === 0){
            console.error('Invalid block: Transactions empty');
            throw new BlockAssertionError('Invalid transactions: expected non-empty set');
        }
    ...
    checkTransaction(transaction, referenceBlockchain = this.blocks) {
        ...
        if (!isNotInBlockchain) {
            ...
            // throw new TransactionAssertionError(`Transaction '${transaction.id}' is already in the blockchain`, transaction); // NOT NEEDED?
        }
